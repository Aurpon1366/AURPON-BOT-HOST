...
def my_bots(message):
    ...
    for b in bots:
        markup.add(types.InlineKeyboardButton(
            f"{b[3]} ({b[5]})", callback_data=f"select_{b[0]}"
        ))
...
def process_deploy(message, bots):
    try:
        ...
        bot_name = bots[choice][3]
        filename = bots[choice][4]
...
logging.basicConfig(level=logging.importlogger = logging.getLogger(__name__)
...
def handle_callbacks(call):
    action, bot_id = call.data.split('_')
    bot_id = int(bot_id)
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=Config.PORT, debug=False)import tweepy
import discord
import asyncio
